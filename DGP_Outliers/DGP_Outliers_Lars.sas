...
libname Lib "C:\Users\dutau\Downloads\SAS project\DGP_Outliers\Lars";
...
model y = X1-X50 / selection = Lar(choose = AIC stop=SBC);      /*Training on the first 50 variables */
run;
...
model y = X1-X50 / selection = Lar(choose = SBC stop=SBC);          /*Training on the first 50 variables */
run;
...
model y = X1-X50 / selection = Lar(choose = CV stop=SBC);      /*Training on the first 50 variables */
run;
