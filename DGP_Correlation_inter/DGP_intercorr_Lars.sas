...
libname Lib "C:\Users\dutau\Downloads\SAS project\DGP_intercorr\Lars";
...
model y = X1-X50 / selection = Lar(choose = AIC stop=CV);      /*Training on the first 50 variables */
run;
...
model y = X1-X50 / selection = Lar(choose = SBC stop=CV);          /*Training on the first 50 variables */
run;
...
model y = X1-X50 / selection = Lar(choose = CV stop=CP);      /*Training on the first 50 variables */
run;
...
model y = X1-X50 / selection = Lar(lscoeffs choose = PRESS stop=SBC);      /*Training on the first 50 variables */
run;
