...
libname Lib "C:\Users\dutau\Downloads\SAS project\DGP_intercorr\Elasticnet";
rsubmit process=task1 wait=no sascmd='sas';
libname Lib "C:\Users\dutau\Downloads\SAS project\DGP_intercorr\Elasticnet";
						   /**********************************************************************/
						   /*********************                            *********************/
                           /*********************    DGP intercorrelation    *********************/
						   /*********************                            *********************/
						   /**********************************************************************/
...
/* Initializing the variance-covariance matrix */
B = do(0.01,1,0.01);
corr = j(100,1,0)`;
do i = 1 to (ncol(B));
corr[i]=B[(ncol(B))-i+1]; /*mirror of B */
end; 
/*We generate the correlation vector used in Toeplitz*/
do i = 2 to (ncol(B));
corr[i]=corr[i]/(1+0.3*i); /*We create a vector with decreasing coefficients */
end;
/* Initializing the model */
do i = 1 to 1000;
varcov = toeplitz(corr);
mean =J(100,1,0);
X = RandNormal(100,mean,varcov);
beta={1,2.1,0.6,2,1.5,2.9};
Y=X[,1:6]*beta+normal(J(100,1,1))*0.1;
...
model y = X1-X50 / selection = Elasticnet(choose = AIC stop=CV);      /*Training on the first 50 variables */
run;
...
model y = X1-X50 / selection = Elasticnet(choose = SBC stop=CV);          /*Training on the first 50 variables */
run;
...
model y = X1-X50 / selection = Elasticnet(choose = CV stop=CP);      /*Training on the first 50 variables */
run;
...
A = Y||X;
names_all = ("X1":"X100");
names_all = "Y"||names_all;                       /* Stores all variables names */
...
model y = X1-X50 / selection = Elasticnet(choose = CP stop=CV);      /*Training on the first 50 variables */
run;
