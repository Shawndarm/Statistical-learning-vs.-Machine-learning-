...
libname Lib "C:\Users\dutau\Downloads\SAS project\DGP_intercorr\Lasso";
...
Y=X[,1:6]*beta+normal(J(100,1,0))*0.1;
...
model y = X1-X50 / selection = Lasso(choose = AIC stop=CV);      /*Training on the first 50 variables */
run;
...
model y = X1-X50 / selection = Lasso(choose = SBC stop=CV);          /*Training on the first 50 variables */
run;
...
model y = X1-X50 / selection = Lasso(choose = CV stop=CP);      /*Training on the first 50 variables */
run;
