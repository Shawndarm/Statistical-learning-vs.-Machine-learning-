...
libname Lib "C:\Users\dutau\Downloads\SAS project\DGP_intracorr\Elasticnet";
...
model y = X1-X50 / selection = Elasticnet(choose = AIC stop=SBC);      /*Training on the first 50 variables */
run;
...
model y = X1-X50 / selection = Elasticnet(choose = SBC stop=CP);          /*Training on the first 50 variables */
run;
...
model y = X1-X50 / selection = Elasticnet(choose = CP stop = CP );      /*Training on the first 50 variables */
run;
